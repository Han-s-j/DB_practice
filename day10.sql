...
FROM member
WHERE REGEXP_LIKE(mem_comtel, '^..-'); -- '.. (╣н╠шюз) -' фпео
...
-- *:0х╦ or ╠в юл╩С х╫╪Ж╥н, ?:0х╦ or 1х╦, +: 1х╦ or ╠вюл╩С
-- mem_addr2 гя╠ш╦╦ юж╢б аж╪р╦╕ ╟к╩Жго╫ц©ю.
SELECT mem_name, mem_add2
FROM member
WHERE REGEXP_LIKE(mem_add2, '^[╟║-фR]*$'); -- +╣╣ ╟А╟З╟║ ╟╟ю╫
-- гя╠шюл гя╠шюз╣╣ ╬Ь╢б
SELECT mem_name, mem_add2
FROM member
WHERE NOT REGEXP_LIKE(mem_add2, '[╟║-фR]');
--WHERE REGEXP_LIKE(mem_add2, '^[^╟║-фR]*$'); -- ю╖©м ╟А╟З ╟╟ю╫
-- | : ╤г╢б, ():╠в╥Л
-- J╥н ╫цюш╣г╦Г, ╪╪╧Ьб╟ ╧╝юз╟║ m or n юн аВ©Ьюг юл╦╖а╤х╦
SELECT emp_name
FROM employees
...
-- юл╦чюо ╠Баь ╬у ╣з цБ╥б
SELECT mem_mail
     , REGEXP_SUBSTR(mem_mail, '[^@]+', 1, 1) as ╬фюл╣П    -- ╟Я╧Пюл╟║ ╬ф╢я 1╥н го╦И ╟Я╧Пюл ╬у
     , REGEXP_SUBSTR(mem_mail, '[^@]+', 1, 2) as ╣╣╦чюн    -- ╟Я╧Пюл╟║ ╬ф╢я 2╥н го╦И ╟Я╧Пюл ╣з
FROM member;
...
SELECT mem_name, mem_id  -- ╫ц╣╣, ╠╨╠╦ цБ╥бго╫ц©ю (mem_id:p001╩╘╟М)
...
-- REGEXP_REPLACE ╢К╩С ╧╝юз©╜ю╩ а╓╠тг╔гЖ╫д фпеою╩ юШ©Кго©╘ ╢ы╦╔ фпеою╦╥н (д║х╞)
SELECT REGEXP_REPLACE('Ellen Hidi Smith','(.*) (.*) (.*)','\3, \1 \2') as re -- ╪╪╧Ьб╟ ╠в╥Люл ╦г ╬ую╦╥н ╠в╦╝╟М','╠НаЖ
FROM dual;
...
-- юЭх╜╧Ьхё ╣чюз╦╝ ╣©юо╧Ьхё (2╟Ё╬©) ╧щ╨╧╣г╢б ╩Г©Ь а╤х╦
