...
EXCEPTION   WHEN NO_DATA_FOUND THEN
           DBMS_OUTPUT.PUT_LINE('데이터 찾을 수 없음.');  -- NO_DATA_FOUND 오류가 발생했을 때 출력
            WHEN ZERO_DIVIDE THEN   -- EXCEPTION : 특정 오류가 발생했을 때 처리하는 부분
           DBMS_OUTPUT.PUT_LINE('오류 zero divide');  -- ZERO_DIVIDE 오류가 발생했을 때 출력
            WHEN OTHERS THEN  
                 DBMS_OUTPUT.PUT_LINE('오류발생!');
                 DBMS_OUTPUT.PUT_LINE('SQLCODE');   --  어떤오류가 난건지 코드로 확인
                 DBMS_OUTPUT.PUT_LINE('SQLERRM');   -- 에러 메세지
END;
...
EXEC ch10_ins_emp_proc ('홍길동', 999); -- 없는 부서 사용자 정의 예외처리 됨.
EXEC ch10_ins_emp_proc ('홍길동', 20); -- 있는 부서 정상처리되어 직원이 저장 됨.
...
SELECT *
FROM  error_log ;   -- 오류가 저장 되어있음(오류체크가능)
